...
export function RecipeEditor({
  ...
  // Serialized payload the action parses — blank rows are dropped so trailing
  // empties don't trip validation.
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <form action={action} className="flex flex-col gap-4 rounded-xl border p-4">
        ...
        <input type="hidden" name="recipe" value={payload} />
        {state?.error && (
...
function PastePanel({
  ...
  onParsed: (parsed: { ingredients: RecipeIngredient[]; instructions: { text: string }[] }) => void
}) {
  const { dict } = useT()
